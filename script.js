 ...
 */ !function(){if(window.__PERF_SHIMS_APPLIED__)return;window.__PERF_SHIMS_APPLIED__=!0;let e=window.requestAnimationFrame?window.requestAnimationFrame.bind(window):e=>setTimeout(e,16),t=new Set(["touchstart","touchmove","wheel","mousewheel"]),r=new Set(["scroll","resize","mousemove"]);function $(t){let r=!1,$,o;return function _(){$=arguments,o=this,r||(r=!0,e(function(){r=!1,t.apply(o,$)}))}}Object.defineProperty(window,"rafThrottle",{value:$,configurable:!0});let o=EventTarget.prototype.addEventListener,_=EventTarget.prototype.removeEventListener,l=new WeakMap;EventTarget.prototype.addEventListener=function(e,_,n){if(!_)return o.call(this,e,_,n);let i=_;"function"==typeof _&&r.has(e)&&(i=$(_),l.set(_,i));let s=n;try{!t.has(e)||(null==n?s={passive:!0}:"boolean"==typeof n?s={capture:n,passive:!0}:"object"!=typeof n||"passive"in n||(s=Object.assign({},n,{passive:!0})))}catch(d){s=n}return o.call(this,e,i,s)},EventTarget.prototype.removeEventListener=function(e,t,r){let $=l.get(t);return _.call(this,e,$||t,r)}}();const{useState:e,useEffect:t,useRef:r,useCallback:$}=React,useInteractiveCard=()=>{t(()=>{let e=document.querySelectorAll(".interactive-card"),t=e=>{let t=e.currentTarget,r=t.getBoundingClientRect(),$=e.clientX-r.left,o=e.clientY-r.top,_=($-r.width/2)/6,l=-((o-r.height/2)/6),n=t.classList.contains("featured-card-js"),i=n?1.15:1.05;t.style.transform=`perspective(1000px) rotateX(${l}deg) rotateY(${_}deg) scale(${i})`,t.style.setProperty("--mouse-x",`${$}px`),t.style.setProperty("--mouse-y",`${o}px`)},r=e=>{let t=e.currentTarget,r=t.classList.contains("featured-card-js"),$=r?1.1:1;t.style.transform=`perspective(1000px) rotateX(0) rotateY(0) scale(${$})`};return e.forEach(e=>{e.style.transition="transform 0.25s cubic-bezier(0.25, 0.46, 0.45, 0.94)",e.addEventListener("mousemove",t),e.addEventListener("mouseleave",r)}),()=>{e.forEach(e=>{e.removeEventListener("mousemove",t),e.removeEventListener("mouseleave",r)})}},[])},useFadeInSection=()=>{t(()=>{let e=document.querySelectorAll(".fade-in-section"),t=new IntersectionObserver(e=>{e.forEach(e=>{e.isIntersecting&&e.target.classList.add("is-visible")})},{threshold:.1});return e.forEach(e=>t.observe(e)),()=>{e.forEach(e=>{e&&t.unobserve(e)})}},[])},useAnimatedCounter=($,o=2e3)=>{let[_,l]=e(0),n=r(null);return t(()=>{let e=n.current,t=new IntersectionObserver(([r])=>{if(r.isIntersecting){let _=$,n=Date.now(),i=()=>{let e=Date.now(),t=Math.min((e-n)/o,1),r=Math.floor(t*_);l(r),t<1&&requestAnimationFrame(i)};i(),e&&t.unobserve(e)}},{threshold:.5});return e&&t.observe(e),()=>{e&&t.unobserve(e)}},[$,o]),[n,_]},useActiveNav=r=>{let[$,o]=e("home");return t(()=>{let e=Array.from(document.querySelectorAll("main section[id]")),t=()=>{let t=window.scrollY+window.innerHeight/3,r=e.map(e=>({id:e.id,offsetTop:e.offsetTop})).filter(e=>e.offsetTop<=t).pop();o(r?r.id:"home")};return window.addEventListener("scroll",t),()=>window.removeEventListener("scroll",t)},[r]),$},Logo=({onScrollTo:e})=><svg onClick={()=>e("home")}className="h-8 w-auto cursor-pointer"viewBox="0 0 100 100"fill="none"xmlns="http://www.w3.org/2000/svg">
        ...
        <path d="M12 10 L12 90 L28 90 L28 60 L60 90 L75 90 L40 50 L75 10 L60 10 L28 40 L28 10 L12 10 Z"className="fill-theme-primary stroke-theme-primary"strokeWidth="4"/>
    ...
    </svg>,DiscordCounter=()=>{let[r,$]=e(null);return t(()=>{let e=()=>{let e=`https://api.allorigins.win/get?url=${encodeURIComponent("https://discord.com/api/guilds/1357439616877072545/widget.json")}`;fetch(e).then(e=>{if(!e.ok)throw Error("Network response was not ok.");return e.json()}).then(e=>{let t=JSON.parse(e.contents);t.code&&t.message?$("N/A"):void 0!==t.presence_count?$(t.presence_count):$("N/A")}).catch(e=>{console.error("Error fetching Discord data:",e),$("Error")})};e();let t=setInterval(e,3e4);return()=>clearInterval(t)},[]),<div className="mt-4 text-lg text-theme-secondary">

            Join <span className="font-bold text-klar">{null===r?"...":r}</span> members online now!

        </div>},AuroraBackground=()=>{let[$]=e(()=>Array.from({length:15}).map(()=>({top:`${100*Math.random()}%`,left:`${100*Math.random()}%`,size:`${Math.floor(300*Math.random()+200)}px`,parallaxFactor:.5*Math.random()+.2}))),o=r($.map(()=>React.createRef()));return t(()=>{let e=()=>{let e=window.scrollY;o.current.forEach((t,r)=>{t.current&&(t.current.style.transform=`translateY(${e*$[r].parallaxFactor}px)`)})};return window.addEventListener("scroll",e,{passive:!0}),()=>window.removeEventListener("scroll",e)},[$]),<div className="aurora-background">

            {$.map((e,t)=><div key={t}ref={o.current[t]}className="aurora-spot"style={{top:e.top,left:e.left,width:e.size,height:e.size}}/>)}

        </div>},Modal=({children:r,onClose:$})=>{let[o,_]=e(!1);t(()=>{_(!0);let e=e=>"Escape"===e.key&&l();return document.body.style.overflow="hidden",document.addEventListener("keydown",e),()=>{document.body.style.overflow="auto",document.removeEventListener("keydown",e)}},[]);let l=()=>{_(!1),setTimeout($,300)};return<div onClick={l}className={`fixed inset-0 z-50 flex items-center justify-center p-4 transition-opacity duration-300 ${o?"opacity-100":"opacity-0"}`}>

             <div className="absolute inset-0 bg-black/80 backdrop-blur-sm"></div>

             <div onClick={e=>e.stopPropagation()}className={`relative transition-all duration-300 ${o?"opacity-100 scale-100":"opacity-0 scale-95"}`}>

                 {r(l)}

             </div>

        </div>},VideoModal=({videoUrls:t,onClose:r})=>{let[$,o]=e(0),_=e=>{let t=null;try{let r=/^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|\&v=)([^#\&\?]*).*/,$=e.match(r);t=$&&11===$[2].length?$[2]:null}catch(o){console.error("Invalid URL:",e,o)}return t},l=()=>o(e=>0===e?t.length-1:e-1),n=()=>o(e=>e===t.length-1?0:e+1);return<Modal onClose={r}>

            {e=><div className="w-screen h-screen flex items-center justify-center relative group">

                    <button onClick={e}className="absolute top-6 right-6 z-50 bg-black/50 text-white rounded-full w-10 h-10 flex items-center justify-center font-bold text-2xl hover:bg-black/80 transition-colors">×</button>

                    <button onClick={l}className="absolute left-4 md:left-16 top-1/2 -translate-y-1/2 bg-black/40 text-white rounded-full w-14 h-14 flex items-center justify-center transition-all z-40 hover:bg-black/70 hover:scale-110">

                        <svg className="w-8 h-8"fill="none"viewBox="0 0 24 24"stroke="currentColor"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth="2"d="M15 19l-7-7 7-7"/></svg>

                    </button>

                    <button onClick={n}className="absolute right-4 md:right-16 top-1/2 -translate-y-1/2 bg-black/40 text-white rounded-full w-14 h-14 flex items-center justify-center transition-all z-40 hover:bg-black/70 hover:scale-110">
                    ...
                    <div className="w-full h-full flex items-center justify-center perspective-1000">
                        ...
                        <div className="relative w-full h-[60vh] flex items-center justify-center transform-style-3d">
                             ...
                             {t.map((e,r)=>{let l=_(e),n=l?`https://img.youtube.com/vi/${l}/hqdefault.jpg`:"https://placehold.co/1280x720/121212/A0A0A0?text=Video",i=r-$,s=t.length;Math.abs(i)>s/2&&(i=i>0?i-s:i+s);let d=0===i,c=1>=Math.abs(i),u={transform:`translateX(${80*i}%) scale(${d?1:.7}) rotateY(${-(40*i)}deg)`,opacity:c?d?1:.3:0,zIndex:s-Math.abs(i),pointerEvents:d?"auto":"none",filter:d?"blur(0px)":"blur(5px)",transition:"all 0.5s cubic-bezier(0.25, 0.46, 0.45, 0.94)"};return<div key={r}className="absolute w-[70%] md:w-[60%] aspect-video"style={u}>

                                        {d?<iframe className="w-full h-full rounded-lg shadow-2xl border-2 border-klar"src={e}title={`Klar Hub Demo Video ${r+1}`}frameBorder="0"allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"allowFullScreen></iframe>:<div className="w-full h-full cursor-pointer"onClick={()=>c&&o(r)}style={{pointerEvents:c?"auto":"none"}}>

                                                <img src={n}className="w-full h-full object-cover rounded-lg shadow-lg"alt={`Video thumbnail ${r+1}`}/>
        ...
        </Modal>},GameFeaturesModal=({game:e,onClose:t})=><Modal onClose={t}>

            {t=><div className="bg-theme-modal-card rounded-lg shadow-2xl w-full max-w-lg border border-theme">

                     <div className="p-4 border-b border-theme flex justify-between items-center">

                         <h3 className="text-xl font-bold text-theme-primary">{e.name} Features</h3>

                         <button onClick={t}className="text-theme-secondary hover:text-theme-primary text-2xl">×</button>
                     ...
                     <div className="p-6">
                         ...
                         <ul className="space-y-3 text-theme-secondary">
                             ...
                             {e.features.map((e,t)=><li key={t}className="flex items-center gap-3">
        ...
        </Modal>,AIHelperModal=({onClose:$})=>{let[o,_]=e(""),[l,n]=e([{role:"ai",text:"Hello! I am the Klar Hub AI assistant. How can I help you today? Feel free to ask about features, pricing, or anything else."}]),[i,s]=e(!1),d=r(null);t(()=>{d.current?.scrollIntoView({behavior:"smooth"})},[l]);let c=async e=>{if(s(!0),"file:"===window.location.protocol){n(e=>[...e,{role:"ai",text:"The AI assistant only works on the live website (klarhub.store). Please visit the site to use this feature."}]),s(!1);return}try{let t=await fetch("/api/gemini",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({prompt:e})});if(!t.ok){let r=await t.text();throw console.error("API proxy error:",r),Error("The AI assistant is experiencing issues.")}let $=await t.json(),o=$.text;if(o)n(e=>[...e,{role:"ai",text:o}]);else throw Error("Received an empty response from the AI assistant.")}catch(_){console.error("AI Helper Error:",_),n(e=>[...e,{role:"ai",text:"Sorry, the AI assistant is currently experiencing issues. Please try again later or join our Discord for help."}])}finally{s(!1)}},u=e=>{e.trim()&&!i&&(n(t=>[...t,{role:"user",text:e}]),c(e),_(""))},m=e=>{e.preventDefault(),u(o)},h=["What are the features for FF2?","How much is lifetime access?","Is Klar Hub safe to use?"];return<Modal onClose={$}>

            {e=><div className="bg-theme-modal-card rounded-lg shadow-2xl w-full max-w-2xl h-[80vh] flex flex-col border border-theme">
                    ...
                    <div className="p-4 border-b border-theme flex justify-between items-center flex-shrink-0">
                        ...
                        <button onClick={e}className="text-theme-secondary hover:text-theme-primary text-2xl">×</button>

                    </div>

                    <div className="flex-1 p-4 space-y-4 overflow-y-auto custom-scrollbar">

                        {l.map((e,t)=><div key={t}className={`flex ${"user"===e.role?"justify-end":"justify-start"}`}>

                                <div className={`max-w-md p-3 rounded-lg ${"user"===e.role?"bg-klar text-white":"bg-theme-button-secondary text-theme-button-secondary-text"}`}>

                                    <p dangerouslySetInnerHTML={{__html:e.text.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>").replace(/\* (.*?)(?:\n|$)/g,"<li>$1</li>").replace(/<li>/g,'<li class="list-disc ml-4">')}}></p>

                                </div>

                            </div>)}

                        {i&&<div className="flex justify-start">
                     ...
                     {1===l.length&&<div className="p-4 border-t border-theme flex-shrink-0">

                             <p className="text-sm text-theme-secondary mb-2 text-center">Or try one of these:</p>

                             <div className="flex flex-wrap justify-center gap-2">

                                 {h.map(e=><button key={e}onClick={()=>u(e)}className="bg-theme-button-secondary hover:bg-theme-button-secondary-hover text-theme-button-secondary-text text-sm px-3 py-1 rounded-full transition">{e}</button>)}

                             </div>

                         </div>}

                    <form onSubmit={m}className="p-4 border-t border-theme flex gap-2 flex-shrink-0">

                        <o type="text"value={o}onChange={e=>_(e.target.value)}placeholder="Ask a question..."className="w-full bg-theme-button-secondary border border-theme rounded-lg text-theme-primary placeholder-theme-secondary focus:outline-none focus:ring-2 focus:ring-klar p-3"/>

                        <button type="submit"className="bg-klar hover:bg-klar-light text-white font-bold py-2 px-4 rounded-lg flex items-center justify-center"disabled={i}>
        ...
        </Modal>},TosModal=({onClose:e})=><Modal onClose={e}>

            {e=><div className="bg-theme-modal-card rounded-lg shadow-2xl w-full max-w-2xl border border-theme">
        ...
        </Modal>,PreviewAnimation=({onAnimationEnd:e})=>(t(()=>{let t=setTimeout(e,1200);return()=>clearTimeout(t)},[e]),<div className="fixed inset-0 z-[100] bg-black/50 backdrop-blur-sm flex items-center justify-center pointer-events-none">
                ...
                <div className="text-lg text-white font-bold tracking-widest uppercase">Initializing Preview</div>
        ...
        </div>),PreviewModal=({onClose:r})=>{let[o,_]=e("Catching"),[l,n]=e(!1),[i,s]=e({magnet_power:25,magnet_chance:100,arm_size:3,football_size:1,dime_lead:11,mag_lead:12.5,bullet_lead:4,lead_distance:0,height_distance:0,walkspeed_value:20,cframe_speed:0,jump_power_value:50,angle_power:50,hip_height_value:0,gravity_value:196.1,delay_auto_guard:.1,power_auto_boost:0,swat_distance:0,prediction_delay:0,hump_speed:5,underground_size:.001,fps_cap:60}),[d,c]=e(null),u=e=>{e!==o&&(n(!0),setTimeout(()=>{_(e),n(!1)},150))},m=$((e,t)=>{s(r=>({...r,[e]:t}))},[]);t(()=>{let e=e=>{d&&(1===e.key.length&&e.key.match(/[a-zA-Z0-9]/i)?m(`${d}_bind`,e.key.toUpperCase()):" "===e.key?m(`${d}_bind`,"Space"):m(`${d}_bind`,e.key),c(null),e.preventDefault())};return d&&window.addEventListener("keydown",e),()=>{window.removeEventListener("keydown",e)}},[d,m]),t(()=>{let e=e=>{let t=" "===e.key.toUpperCase()?"SPACE":e.key.toUpperCase();Object.keys(i).forEach(e=>{if(e.endsWith("_bind")&&i[e]&&i[e].toUpperCase()===t){let r=e.replace("_bind","_enabled");i.hasOwnProperty(r)&&m(r,!i[r])}})};return window.addEventListener("keydown",e),()=>{window.removeEventListener("keydown",e)}},[i,m]);let h=[{name:"Catching",icon:<svg xmlns="http://www.w3.org/2000/svg"className="h-5 w-5"viewBox="0 0 20 20"fill="currentColor"><path fillRule="evenodd"d="M14.707 10.293a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 111.414-1.414L9 12.586V5a1 1 0 012 0v7.586l2.293-2.293a1 1 0 011.414 0z"clipRule="evenodd"/><path d="M5 3a1 1 0 000 2h10a1 1 0 100-2H5z"/></svg>},{name:"Throwing",icon:<svg xmlns="http://www.w3.org/2000/svg"className="h-5 w-5"viewBox="0 0 20 20"fill="currentColor"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"/></svg>},{name:"Player",icon:<svg xmlns="http://www.w3.org/2000/svg"className="h-5 w-5"viewBox="0 0 20 20"fill="currentColor"><path fillRule="evenodd"d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z"clipRule="evenodd"/></svg>},{name:"Automatic",icon:<svg xmlns="http://www.w3.org/2000/svg"className="h-5 w-5"viewBox="0 0 20 20"fill="currentColor"><path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z"/></svg>},{name:"Physic",icon:<svg xmlns="http://www.w3.org/2000/svg"className="h-5 w-5"viewBox="0 0 20 20"fill="currentColor"><path fillRule="evenodd"d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.38z"clipRule="evenodd"/></svg>},{name:"Visual",icon:<svg xmlns="http://www.w3.org/2000/svg"className="h-5 w-5"viewBox="0 0 20 20"fill="currentColor"><path d="M10 12a2 2 0 100-4 2 2 0 000 4z"/><path fillRule="evenodd"d="M.458 10C3.732 4.943 9.522 3 10 3s6.268 1.943 9.542 7c-3.274 5.057-9.064 7-9.542 7S3.732 15.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z"clipRule="evenodd"/></svg>},{name:"Trolling",icon:<svg xmlns="http://www.w3.org/2000/svg"className="h-5 w-5"viewBox="0 0 20 20"fill="currentColor"><path fillRule="evenodd"d="M10 18a8 8 0 100-16 8 8 0 000 16zM7 9a1 1 0 100-2 1 1 0 000 2zm7-1a1 1 0 11-2 0 1 1 0 012 0zm-.464 5.535a1 1 0 10-1.415-1.414 3 3 0 01-4.242 0 1 1 0 00-1.415 1.414 5 5 0 007.072 0z"clipRule="evenodd"/></svg>},{name:"UI Settings",icon:<svg xmlns="http://www.w3.org/2000/svg"className="h-5 w-5"viewBox="0 0 20 20"fill="currentColor"><path fillRule="evenodd"d="M11.49 3.17c-.38-1.56-2.6-1.56-2.98 0a1.532 1.532 0 01-2.286.948c-1.372-.836-2.942.734-2.106 2.106.54.886.061 2.042-.947 2.287-1.561.379-1.561 2.6 0 2.978a1.532 1.532 0 01.947 2.287c-.836 1.372.734 2.942 2.106 2.106a1.532 1.532 0 012.287.947c.379 1.561 2.6 1.561 2.978 0a1.533 1.533 0 012.287-.947c1.372.836 2.942-.734 2.106-2.106a1.533 1.533 0 01-.947-2.287c1.561-.379 1.561-2.6 0-2.978a1.532 1.532 0 01-.947-2.287c.836-1.372-.734-2.942-2.106-2.106a1.532 1.532 0 01-2.287-.947zM10 13a3 3 0 100-6 3 3 0 000 6z"clipRule="evenodd"/></svg>},],b=()=>{switch(o){case"Catching":return<>
                        ...
                        <FeatureCard id="magnets"title="Magnets"icon={<svg className="w-4 h-4"viewBox="0 0 20 20"fill="currentColor"><path d="M15.042 21.672L13.684 16.6m0 0l-2.51 2.225.569-9.47 5.227 7.917-3.286-.672zm-7.518-.267A8.25 8.25 0 1120.25 10.5M8.288 14.212A5.25 5.25 0 1117.25 10.5"/></svg>}>
                         ...
                         <FeatureCard id="qb_settings"title="QB Settings"icon={<svg className="w-4 h-4"viewBox="0 0 20 20"fill="currentColor"><path d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5"/></svg>}>
                            ...
                            <Slider id="mag_lead"label="Mag Lead"min={0}max={20}step={.1}/>
                        ...
                        <FeatureCard id="gravity"title="Gravity"icon={<svg className="w-4 h-4"viewBox="0 0 20 20"fill="currentColor"><path d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z"/></svg>}><Checkbox id="gravity_enabled"label="Gravity"/><Slider id="gravity_value"label="Gravity"min={0}max={500}step={.1}/></FeatureCard>
                        ...
                        <FeatureCard id="auto_guard"title="Auto Guard"icon={<svg className="w-4 h-4"viewBox="0 0 20 20"fill="currentColor"><path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z"/></svg>}><Checkbox id="auto_guard_enabled"label="Auto Guard"/><Slider id="delay_auto_guard"label="Delay"min={0}max={1}step={.1}/></FeatureCard>
                        ...
                        <FeatureCard id="head_size"title="Head Size"icon={<svg className="w-4 h-4"viewBox="0 0 20 20"fill="currentColor"><path d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.38z"/></svg>}><Checkbox id="head_size_enabled"label="Head Size"/><Slider id="resize_head"label="Resize Head"min={1}max={5}step={.1}/><Slider id="transparency"label="Transparency"min={0}max={1}step={.1}/></FeatureCard>
                    ...
                    </>;case"UI Settings":return<div className="col-span-2">
                    ...
                    </div>;default:return<div className="col-span-2 text-center text-gray-500 pt-10">Select a tab from the left.</div>}};return<Modal onClose={r}>

            {e=><div className="w-[800px] h-[500px] bg-[#0D0D0F] text-white rounded-lg flex overflow-hidden border border-gray-800 shadow-2xl shadow-black/50">
                    ...
                    <div className="w-48 bg-[#18181C] p-4 flex flex-col">
                        ...
                        <div className="mt-6 flex-grow space-y-1">
                            ...
                            {h.map(e=><button key={e.name}onClick={()=>u(e.name)}className={`w-full flex items-center gap-3 px-3 py-2 text-sm rounded-md transition-colors relative ${o===e.name?"text-white bg-klar/10":"text-gray-400 hover:bg-white/5"}`}>

                                    {e.icon}

                                    <span >{e.name}</span>

                                    {o===e.name&&<div className="absolute left-0 top-1/2 -translate-y-1/2 h-5 w-1 bg-klar rounded-r-full"></div>}
                    ...
                    <div className={`flex-1 p-6 overflow-y-auto custom-scrollbar transition-opacity duration-150 ${l?"opacity-0":"opacity-100"}`}>
        ...
        </Modal>},ComparePlansModal=({onClose:e,allTiers:t})=>{let r=[{name:"Price",key:"price"},{name:"Robux Price",key:"robuxPrice"},{name:"Duration",getValue:e=>e.name.includes("Lifetime")?"Lifetime":e.name.includes("1 Week")?"7 Days":e.name.includes("1 Month")?"30 Days":e.name.includes("3 Month")?"90 Days":e.name.includes("6 Month")?"180 Days":"N/A"},{name:"Access To All Games",getValue:e=>e.name.includes("Klar")?"✔️":"N/A"},{name:"Premium Support",getValue:e=>"✔️"},];return<Modal onClose={e}>

            {e=><div className="bg-theme-modal-card rounded-lg shadow-2xl w-full max-w-4xl border border-theme">
                        ...
                        <table className="w-full text-left border-collapse">
                            ...
                            <thead >
                                ...
                                <tr >
                                    ...
                                    {t.map(e=><th key={e.name}className="p-3 text-sm font-semibold text-theme-primary bg-theme-dark text-center whitespace-nowrap">

                                            {e.name}

                                            {e.isFeatured&&<span className="block text-xs text-klar font-normal">(Best Value)</span>}
                            ...
                            <tbody >
                                ...
                                {r.map((e,r)=><tr key={e.name}className="border-b border-theme">

                                        <td className="p-3 font-medium text-theme-secondary">{e.name}</td>

                                        {t.map(t=><td key={t.name}className="p-3 text-center text-theme-primary">

                                                {e.getValue?e.getValue(t):t[e.key]||"N/A"}
                                ...
                                <tr className="">
                                    ...
                                    {t.map(e=><td key={e.name}className="p-3 text-center">

                                            <a href={e.url}target="_blank"rel="noopener noreferrer"className="inline-block w-full py-2 px-4 rounded-lg font-semibold text-center transition bg-klar/20 hover:bg-klar/30 text-klar border border-klar text-sm">
        ...
        </Modal>},Header=({headerRef:e,onScrollTo:t,onToggleMobileMenu:r,onTosClick:$,activeSection:o,isMobileMenuOpen:_,onGameClick:l,theme:n,setTheme:i})=>{let s=[{id:"features",label:"Features"},{id:"games",label:"Supported Games"},{id:"pricing",label:"Pricing"},{id:"free",label:"Free Access"},{id:"reviews",label:"Reviews"},{id:"faq",label:"FAQ"},{id:"tos",label:"Terms"}];return<header ref={e}className="bg-theme-header sticky top-0 z-40 p-4 flex justify-between items-center backdrop-blur-sm transition-colors duration-300">
            ...
            <nav className="hidden md:flex flex-shrink-0 justify-center items-center gap-6 text-sm font-semibold">
                ...
                {s.map(e=><button key={e.id}onClick={()=>"tos"===e.id?$():t(e.id)}className={`text-theme-secondary hover:text-klar transition ${o===e.id?"nav-active":""}`}>
            ...
            <div className="flex-1 hidden md:flex justify-end items-center gap-4">
                ...
                <button onClick={()=>i("dark"===n?"light":"dark")}className="p-2 rounded-full bg-theme-button-secondary hover:bg-theme-button-secondary-hover transition"aria-label="Toggle theme">

                    {"dark"===n?<svg className="w-6 h-6 text-yellow-300"fill="none"viewBox="0 0 24 24"stroke="currentColor"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth="2"d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"/></svg>:<svg className="w-6 h-6 text-theme-primary"fill="none"viewBox="0 0 24 24"stroke="currentColor"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth="2"d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"/></svg>}

                </button>

                <a href={"https://discord.gg/bGmGSnW3gQ"}target="_blank"rel="noopener noreferrer"className="inline-block py-2 px-6 rounded-lg font-semibold text-center transition bg-klar/20 hover:bg-klar/30 text-klar border border-klar">Join Discord</a>

            </div>

            <div className="md:hidden flex-1 flex justify-end">

                <button onClick={r}className="text-theme-primary z-50">

                    {_?<svg className="w-6 h-6"fill="none"viewBox="0 0 24 24"stroke="currentColor"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth="2"d="M6 18L18 6M6 6l12 12"/></svg>:<svg className="w-6 h-6"fill="none"viewBox="0 0 24 24"stroke="currentColor"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth="2"d="M4 6h16M4 12h16m-7 6h7"/></svg>}

                </button>

            </div>

        </header>},MobileMenu=({isOpen:e,onScrollTo:t,onTosClick:r,onClose:$})=>{if(!e)return null;let o=[{id:"features",label:"Features"},{id:"games",label:"Supported Games"},{id:"pricing",label:"Pricing"},{id:"free",label:"Free Access"},{id:"reviews",label:"Reviews"},{id:"faq",label:"FAQ"},{id:"tos",label:"Terms"}];return<div className="fixed top-0 left-0 w-full h-full z-30 bg-theme-dark/95 backdrop-blur-lg flex flex-col items-center justify-center gap-8 text-2xl font-bold md-hidden">

            {o.map(e=><button key={e.id}onClick={()=>{"tos"===e.id?r():t(e.id),$()}}className="text-theme-secondary hover:text-klar transition">{e.label}</button>)}

            <div className="mt-4"><a href={"https://discord.gg/bGmGSnW3gQ"}target="_blank"rel="noopener noreferrer"className="inline-block py-3 px-8 text-xl rounded-lg font-semibold text-center transition bg-klar hover:bg-klar-light text-white">Join Discord</a></div>

        </div>},BackToTopButton=()=>{let[r,$]=e(!1);return t(()=>{let e=()=>{window.scrollY>300?$(!0):$(!1)};return window.addEventListener("scroll",e),()=>window.removeEventListener("scroll",e)},[]),<button id="back-to-top"onClick={()=>window.scrollTo({top:0,behavior:"smooth"})}className={`fixed bottom-8 left-8 bg-klar/80 hover:bg-klar text-white w-12 h-12 rounded-full flex items-center justify-center pointer-events-auto transition-all ${r?"opacity-100 translate-y-0":"opacity-0 translate-y-4 pointer-events-none"}`}>
        ...
        </button>},AIHelperButton=({onClick:r})=>{let[$,o]=e(!0);return t(()=>{let e=setTimeout(()=>o(!1),7e3);return()=>clearTimeout(e)},[]),<div className="action-button-wrapper fixed bottom-8 right-8 z-40">
            ...
            <button id="ai-helper-button"onClick={r}className="bg-klar/80 hover:bg-klar text-white w-12 h-12 rounded-full flex items-center justify-center pointer-events-auto shadow-lg shadow-klar">
        ...
        </div>},Footer=()=><footer className="w-full p-8 text-center text-gray-500 text-sm">
     ...
     </footer>,App=()=>{let[$,o]=e(!1),[_,l]=e(!1),[n,i]=e(!1),[s,d]=e(null),[c,u]=e(!1),[m,h]=e(null),[b,x]=e(!1),[f,g]=e(!1),[y,w]=e(!1),[v,k]=e(!1),[M,z]=e(""),[C,A]=e(()=>localStorage.getItem("klar-theme")||"dark");t(()=>{let e=document.documentElement;localStorage.setItem("klar-theme",C);let t={dark:{"--background-dark":"#121212","--background-light":"#1E1E1E","--text-primary":"#F9FAFB","--text-secondary":"#D1D5DB","--border-color":"#4B5563","--header-bg":"rgba(18, 18, 18, 0.7)","--card-bg":"rgba(30, 30, 30, 0.4)","--modal-card-bg":"#1F2937","--button-secondary-bg":"#374151","--button-secondary-hover-bg":"#4B5563","--button-secondary-text":"#F9FAFB","--aurora-opacity":"0.1"},light:{"--background-dark":"#F9FAFB","--background-light":"#FFFFFF","--text-primary":"#111827","--text-secondary":"#374151","--border-color":"#9CA3AF","--header-bg":"rgba(249, 250, 251, 0.8)","--card-bg":"#FFFFFF","--modal-card-bg":"#FFFFFF","--button-secondary-bg":"#E5E7EB","--button-secondary-hover-bg":"#D1D5DB","--button-secondary-text":"#111827","--aurora-opacity":"0.05"}},r=t[C];for(let[$,o]of Object.entries(r))e.style.setProperty($,o)},[C]),t(()=>{window.scrollTo(0,0);let e=document.getElementById("preloader");setTimeout(()=>{e&&e.classList.add("loaded")},1e3)},[]),useFadeInSection(),useInteractiveCard();let F=r(null),[P,S]=e(80),L=useActiveNav(P);t(()=>{F.current&&S(F.current.offsetHeight)},[]);let[H,T]=useAnimatedCounter(80),[j,E]=useAnimatedCounter(20),[D,R]=useAnimatedCounter(99),B=e=>{let t=document.getElementById(e);if(t){let r=t.offsetTop-P;window.scrollTo({top:r,behavior:"smooth"}),history.pushState(null,null,`#${e}`)}i(!1)};t(()=>{if(P>0&&80!==P){let e=window.location.hash.replace("#","");if(e){let t=document.getElementById(e);if(t){let r=t.offsetTop-P;window.scrollTo({top:r,behavior:"smooth"})}}}},[P]);let I=()=>{let e=M||"insert key",t=`script_key="${e}";
loadstring(game:HttpGet("https://api.luarmor.net/files/v3/loaders/50da22b3657a22c353b0dde631cb1dcf.lua"))()`;navigator.clipboard.writeText(t).then(()=>{u(!0),setTimeout(()=>u(!1),2e3)}).catch(e=>{console.error("Failed to copy script: ",e)})},V=()=>{w(!0)},q=["https://www.youtube.com/embed/d2hR2gRhME0?autoplay=1","https://www.youtube.com/embed/97osD4zLYpA?autoplay=1","https://www.youtube.com/embed/03Y0NuUEOV8?autoplay=1"],G=[{name:"1 Week Klar Access",price:"$1.50",url:"https://klarhub.sellhub.cx/product/1-Week/",specialTag:"Most Popular"},{name:"Lifetime Klar",price:"$15.00",url:"https://klarhub.sellhub.cx/product/New-product/",isFeatured:!0},{name:"Extreme Alt Gen",price:"$1.00",url:"https://klarhub.sellhub.cx/product/Extreme-Alt-Gen/",specialTag:"On Sale"},{name:"1 Month Klar Access",price:"$2.50",url:"https://klarhub.sellhub.cx/product/1-Month-Klar-Access/",robuxPrice:"450",robuxUrl:"https://www.roblox.com/catalog/116340932269907/KLAR-1-month"},{name:"3 Month Klar Access",price:"$3.75",url:"https://klarhub.sellhub.cx/product/3-Month-Access/",robuxPrice:"800",robuxUrl:"https://www.roblox.com/catalog/71184399134072/KLAR-3-Month"},{name:"6 Month Klar Access",price:"$5.50",url:"https://klarhub.sellhub.cx/product/6-Month-Klar-Access/",robuxPrice:"1225",robuxUrl:"https://www.roblox.com/catalog/134764715699815/KLAR-6-Month"},],U=G.slice(0,3),K=G.slice(3),N=[{name:"Customer",stars:5,text:"easiest checkout",date:"Wed Jul 23 2025"},{name:"Customer",stars:5,text:"Amazing and easy",date:"Wed Jul 16 2025"},{name:"Customer",stars:5,text:"best script out there cop now",date:"Fri Jun 06 2025"}],W=[{q:"Is Klar Hub a one-time purchase?",a:"We offer both subscription and lifetime access plans. You can choose the one that best suits your needs."},{q:"What payment methods are accepted?",a:"We accept all major payment methods through our secure online storefront, including credit cards, PayPal, and more."},{q:"What executors are compatible?",a:"Our scripts are designed to be compatible with all major, high-quality executors on the market."},{q:"How often are the scripts updated?",a:"We update our scripts regularly to ensure compatibility with the latest Roblox updates and to add new features. Updates are always free for active subscribers or lifetime members."}],J=[{title:"Perfect Kicking & Throwing",description:"Achieve perfect accuracy and power on every kick and throw with our advanced precision assistance.",icon:"M15.59 14.37a6 6 0 01-5.84 7.38v-4.82m5.84-2.56v4.82a6 6 0 01-1.292 3.536l-1.992-1.992a4.5 4.5 0 00-6.364-6.364l-1.992-1.992A6 6 0 0115.59 14.37z"},{title:"QB Aimbot",description:"Never miss a throw with our precise quarterback aimbot, featuring prediction and sensitivity controls.",icon:"M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5"},{title:"Ball Magnets",description:"Automatically catch passes with our intelligent ball magnet, ensuring you never drop the ball.",icon:"M15.042 21.672L13.684 16.6m0 0l-2.51 2.225.569-9.47 5.227 7.917-3.286-.672zm-7.518-.267A8.25 8.25 0 1120.25 10.5M8.288 14.212A5.25 5.25 0 1117.25 10.5"}],Q=[{name:"Football Fusion 2",abbr:"FF2",features:["Ball Magnets","Pull Vector","Enhanced Movement (Jump & Speed)","No Jump Cooldown","Custom Catch Effects"]},{name:"Ultimate Football",abbr:"UF",features:["Football Size Manipulation","Arm Resize","Enhanced Movement (Jump & Speed)","No-Clip (Utility)"]},{name:"Murders VS Sheriffs Duels",abbr:"MVSD",features:["Advanced Triggerbot","Hitbox Extender","Enhanced Movement (Jump & Speed)","Player ESP"]},{name:"Arsenal",abbr:"Arsenal",features:["Silent Aim","Advanced Hitbox Manipulation","Triggerbot","Visual Tags (Admin, etc.)"]}];return<div className="bg-theme-dark text-theme-primary">

            <AuroraBackground />

            <div className="relative z-10">

                <Header headerRef={F}onScrollTo={B}onToggleMobileMenu={()=>i(!n)}onTosClick={()=>x(!0)}isMobileMenuOpen={n}activeSection={L}theme={C}setTheme={A}/>

                 <MobileMenu isOpen={n}onScrollTo={B}onTosClick={()=>{x(!0),i(!1)}}onClose={()=>i(!1)}/>
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="mt-8 flex flex-col items-center justify-center gap-4">
                               ...
                               <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                                    ...
                                    <button onClick={()=>B("pricing")}className="py-3 px-8 rounded-lg font-semibold text-center transition bg-klar hover:bg-klar-light text-white shadow-lg shadow-klar flex items-center gap-2">
                                    ...
                                    <button onClick={()=>o(!0)}className="py-3 px-8 rounded-lg font-semibold text-center transition bg-transparent border border-theme text-theme-secondary hover:text-theme-primary hover:border-klar flex items-center gap-2">
                                ...
                                <DiscordCounter />
                                 ...
                                 <button onClick={V}className="mt-2 py-2 px-6 rounded-lg font-semibold text-center transition bg-theme-button-secondary hover:bg-theme-button-secondary-hover text-theme-button-secondary-text flex items-center gap-2">
                    ...
                    <div className="w-full max-w-6xl mx-auto px-4 space-y-24">
                         ...
                         <section id="stats"className="py-12 fade-in-section">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
                                ...
                                <div ref={H}>
                                    ...
                                    <p className="text-5xl font-extrabold text-klar">{T.toLocaleString()}+</p>
                                ...
                                <div ref={j}>
                                    ...
                                    <p className="text-5xl font-extrabold text-klar">{E}+</p>
                                ...
                                <div ref={D}>
                                    ...
                                    <p className="text-5xl font-extrabold text-klar">{R}%</p>
                        ...
                        <section id="features"className="py-12 text-center fade-in-section">
                            ...
                            <div className="mt-12 grid md:grid-cols-3 gap-8">
                                ...
                                {J.map(e=><div key={e.title}className="bg-theme-card p-6 rounded-lg border border-theme text-left interactive-card">

                                         <svg className="w-8 h-8 text-klar mb-4"fill="none"viewBox="0 0 24 24"stroke="currentColor"strokeWidth="1.5"><path strokeLinecap="round"strokeLinejoin="round"d={e.icon}/></svg>

                                         <h4 className="text-xl font-semibold">{e.title}</h4>

                                         <p className="text-theme-secondary mt-2">{e.description}</p>
                        ...
                        <section id="games"className="py-12 text-center fade-in-section">
                             ...
                             <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
                                  ...
                                  {Q.map(e=><div key={e.name}className="bg-theme-card p-8 rounded-lg border border-theme text-center interactive-card flex flex-col justify-between">

                                           <div >

                                               <h4 className="text-2xl font-bold">{e.name}</h4>

                                               <p className="text-klar font-semibold text-lg">{e.abbr}</p>

                                           </div>

                                           <button onClick={()=>h(e)}className="mt-6 w-full py-2 px-4 rounded-lg font-semibold text-center transition bg-klar/20 hover:bg-klar/30 text-klar border border-klar">
                        ...
                        <section id="pricing"className="py-12 text-center fade-in-section">
                            ...
                            <div className="mt-12 grid md:grid-cols-3 gap-8 items-center">
                                ...
                                {U.map(e=><div key={e.name}className={`relative bg-theme-card p-8 rounded-lg border text-center interactive-card flex flex-col ${e.isFeatured?"border-klar shadow-2xl shadow-klar/40 featured-card-js":"border-theme"}`}>

                                        {(e.isFeatured||e.specialTag)&&<div className={`absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 px-4 py-1 text-sm font-semibold text-white rounded-full shadow-md ${e.isFeatured?"bg-klar":"On Sale"===e.specialTag?"bg-red-500":"bg-indigo-500"}`}>

                                                {e.isFeatured?"Best Value":e.specialTag}

                                            </div>}

                                        <h4 className="text-xl font-bold mb-2 h-12 flex items-center justify-center">{e.name}</h4>

                                        <div className="flex justify-center items-end gap-2 mb-4">

                                            <p className="text-4xl font-extrabold text-klar">{e.price}</p>

                                            {e.robuxPrice&&<>

                                                    <span className="text-xl text-theme-secondary pb-1">or</span>

                                                    <p className="text-4xl font-extrabold text-klar">R${e.robuxPrice}</p>
                                        ...
                                        <div className="flex flex-col gap-2 mt-auto">
                                            ...
                                            <a href={e.url}target="_blank"rel="noopener noreferrer"className="inline-block w-full py-2 px-4 rounded-lg font-semibold text-center transition-all duration-300 bg-klar/20 hover:bg-klar/30 text-klar border border-klar hover:shadow-[0_0_15px_var(--klar-primary)]">Purchase (USD)</a>

                                            {e.robuxUrl&&<a href={e.robuxUrl}target="_blank"rel="noopener noreferrer"className="inline-block w-full py-2 px-4 rounded-lg font-semibold text-center transition bg-[#00A2FF]/20 hover:bg-[#00A2FF]/30 text-[#00A2FF] border border-[#00A2FF]">
                            ...
                            <div className="mt-8 grid md:grid-cols-3 gap-8">
                                ...
                                {K.map(e=><div key={e.name}className="relative bg-theme-card p-8 rounded-lg border text-center interactive-card flex flex-col transition-[box-shadow,border-color] duration-300 border-theme">
                            ...
                            <div className="text-center mt-8">
                                ...
                                <button onClick={()=>k(!0)}className="py-2 px-6 rounded-lg font-semibold text-center transition bg-theme-button-secondary hover:bg-theme-button-secondary-hover text-theme-button-secondary-text">
                                        ...
                                        <div className="ml-4 p-6 bg-theme-card border border-theme rounded-lg">
                                            ...
                                            <div className="mt-4 bg-theme-dark p-4 rounded-lg relative">
                                                ...
                                                <div className="mt-4 flex flex-col sm:flex-row gap-2">
                                                    ...
                                                    <input type="text"value={M}onChange={e=>z(e.target.value)}placeholder="Paste your key here"className="w-full bg-theme-button-secondary border border-theme rounded-lg text-theme-primary placeholder-theme-secondary focus:outline-none focus:ring-2 focus:ring-klar p-2"/>

                                                    <button onClick={I}className="flex-shrink-0 bg-klar hover:bg-klar-light text-white px-4 py-2 text-sm font-semibold rounded-lg transition relative overflow-hidden">

                                                        <span className={`flex items-center gap-2 transition-transform duration-300 ${c?"-translate-y-full":"translate-y-0"}`}>
                                                         ...
                                                         <span className={`absolute inset-0 flex items-center justify-center gap-2 transition-transform duration-300 ${c?"translate-y-0":"translate-y-full"}`}>
                        ...
                        <section id="faq"className="py-12 max-w-3xl mx-auto fade-in-section">
                            ...
                            <div className="mt-12 space-y-4">
                                ...
                                {W.map((e,t)=><div key={t}className="bg-theme-card border border-theme rounded-lg faq-item">

                                        <button onClick={()=>d(s===t?null:t)}className="w-full flex justify-between items-center p-6 text-left">

                                            <span className="text-lg font-semibold">{e.q}</span>

                                            <svg className={`w-6 h-6 text-theme-secondary transition-transform duration-300 ${s===t?"rotate-180":""}`}fill="none"viewBox="0 0 24 24"stroke="currentColor"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth="2"d="M19 9l-7 7-7-7"></path></svg>

                                        </button>

                                        <div className="grid transition-all duration-500 ease-in-out"style={{gridTemplateRows:s===t?"1fr":"0fr"}}>

                                            <div className="overflow-hidden"><p className="p-6 pt-0 text-theme-secondary">{e.a}</p></div>
                ...
                {$&&<VideoModal videoUrls={q}onClose={()=>o(!1)}/>}

                <AIHelperButton onClick={()=>l(!0)}/>

                {_&&<AIHelperModal onClose={()=>l(!1)}/>}

                {m&&<GameFeaturesModal game={m}onClose={()=>h(null)}/>}

                {b&&<TosModal onClose={()=>x(!1)}/>}

                {y&&<PreviewAnimation onAnimationEnd={()=>{w(!1),g(!0)}}/>}

                {f&&<PreviewModal onClose={()=>g(!1)}/>}

                {v&&<ComparePlansModal onClose={()=>k(!1)}allTiers={G}/>}

                <BackToTopButton />

            </div>

        </div>},root=ReactDOM.createRoot(document.getElementById("root"));root.render(<App />);
